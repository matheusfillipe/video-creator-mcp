...
Read this BEFORE using `video_render` or `video_render_timeline` with custom HTML. If your brief fits a template tool (slideshow/tierlist/terminal/chart/catalog), the template stamps correct HTML for you — don't read this, just call the template.
...
3. **Layout works at full size?** Before scaling to N segments, render the SAME composition with `data-duration="3"` via `video_render` (a quick sample), extract frames at 0.5s and 2.5s, vision-check: video fills 1920×1080 (no letterboxing), text not cut off, no overlap with baked source-text.
