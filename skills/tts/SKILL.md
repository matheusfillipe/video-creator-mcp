...
- **Sync visuals to the narration → use `video_narrated_scenes`.** When each line should show matching footage (the moon clip while "the moon landing" is spoken), give it ordered `scenes: [{ line, media_id }]`. It generates each line, cuts that scene's footage to the line's exact spoken length, and stitches them, so scene N is on screen exactly while line N is heard — sync guaranteed by construction, no timestamps or alignment, any length. Add `music_media_id` (+ `lead_in_sec`) for a ducked bed and `voice_reference` to clone one voice across all lines. This is the reliable path for a narrated montage; don't hand-chain `video_tts` + `video_add_audio` for that.
