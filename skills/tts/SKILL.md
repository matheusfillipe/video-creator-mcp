...
Every call returns `duration_sec` and a downloadable `url`, plus a `tts-audio` JSON artifact describing the acting used. Patterns:

- **Sync visuals to the narration → use `video_plan` → `video_compose`.** When each line should show matching footage (the moon clip while "the moon landing" is spoken), give the composition one scene per line, each pairing a voice clip (the narration text) with its visual (footage or a math graphic); the scene is cut to that voice clip's exact spoken length, so scene N is on screen exactly while line N is heard — sync guaranteed by construction, no timestamps or alignment, any length. Put music on its own audio clip (it loops and ducks under the voice) and set a `voice.reference_media_id` default to clone one narrator voice across every scene. Validate with `video_plan` first, fix its findings, then call `video_compose` once. This is the reliable path for a narrated montage; don't hand-chain `video_tts` + `video_add_audio` for that.
