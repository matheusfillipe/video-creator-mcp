...
**Narration & sound (TTS / music / ambient)** — video_caption and chrome compositions produce NO audio, so add it as a final step: \`video_tts\` your narration script → it returns an audio \`media_id\` → \`video_add_audio(media_id: "<your video>", audio_media_id: "<tts>", mode: "replace")\` lays the voiceover over the (muted) video. For background music or ambient sound, download it to a media_id and call \`video_add_audio\` again with \`mode: "mix"\` (blends under the narration). So "make a documentary with TTS narration and buzzing" = build the visual (clips + captions) → \`video_tts\` → \`video_add_audio\` replace → \`video_add_audio\` mix. If the user asked for narration/sound, the finished video MUST have an audio track — never hand back a silent video.
