...
describe("audioPlanWarnings", () => {
  ...
  it("passes the shape we actually want: clip keeps its sound, song starts after it", () => {
    expect(audioPlanWarnings([clip()], [track()], 28, hint)).toEqual([]);
  });

  it("flags music laid across footage that kept its sound", () => {
    const out = audioPlanWarnings([clip()], [track({ from: 0, to: 20 })], 28, hint);
  ...
  it("calls out replace over kept footage as throwing the sound away", () => {
    const out = audioPlanWarnings(
      [clip()],
      [track({ from: 0, to: 28, exclusive: true })],
      28,
      hint,
    );
  ...
  it("flags a silenced clip with nothing playing over it", () => {
    const out = audioPlanWarnings([clip({ silent: true })], [track()], 28, hint);
    expect(out.join()).toMatch(/COMPLETELY SILENT/);
  });

  it("accepts a silenced clip when a track covers it", () => {
    const out = audioPlanWarnings([clip({ silent: true })], [track({ from: 0, to: 28 })], 28, hint);
    expect(out.join()).not.toMatch(/COMPLETELY SILENT/);
  });

  it("flags a clip cut short of its source", () => {
    const out = audioPlanWarnings([clip({ to: 8, sourceLen: 10 })], [track({ from: 8 })], 28, hint);
  ...
  it("flags a soundtrack that runs out before the video does", () => {
    const out = audioPlanWarnings([clip()], [track({ from: 13, to: 20 })], 40, hint);
