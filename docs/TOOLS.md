...
| `audio_sync_ms` | number | no |  | Leading silence padded onto the audio to line it up with the video, in ms. Default suits pages that make sound via Web Audio (games, MIDI/soundfont players). Pass 0 for a page that just plays a <video>/<audio> file (already synced) — otherwise its audio lands late. |
